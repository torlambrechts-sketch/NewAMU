...
export function SurveyResponsesTab({ survey, campaign }: Props) {
  const [copied, setCopied] = useState(false)
  const surveyUrl = `${window.location.origin}/survey-respond/${campaign.id}`
  ...
  if (campaign.status === 'draft') {
    return (
      <div className="flex flex-col items-center gap-3 py-16 text-center">
        ...
        <p className="text-sm font-medium text-neutral-500">Undersøkelsen er ikke åpen ennå</p>
        <p className="text-xs text-neutral-400">Åpne undersøkelsen for å aktivere innsamling av besvarelser.</p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        <div className="rounded-xl border border-neutral-200 bg-white p-4 text-center shadow-sm">
          <div className="text-3xl font-bold text-[#1a3d32]">{survey.respondentCount}</div>
          <div className="mt-1 text-xs text-neutral-500">Respondenter</div>
        </div>
        <div className="rounded-xl border border-neutral-200 bg-white p-4 text-center shadow-sm">
          <div className="text-3xl font-bold text-neutral-600">{survey.questions.length}</div>
          <div className="mt-1 text-xs text-neutral-500">Spørsmål</div>
        </div>
        <div className="rounded-xl border border-neutral-200 bg-white p-4 text-center shadow-sm sm:col-span-1 col-span-2">
          <div
            className={`text-3xl font-bold ${campaign.status === 'open' ? 'text-emerald-600' : 'text-blue-600'}`}
          >
            {campaign.status === 'open' ? 'Åpen' : 'Lukket'}
          </div>
      ...
      {campaign.status === 'open' && (
        <div className="space-y-3 rounded-xl border border-neutral-200 bg-white p-5 shadow-sm">
          <h3 className="font-semibold text-[#1a3d32]">Del undersøkelsen</h3>
          <p className="text-sm text-neutral-500">
            Del denne lenken med ansatte. Besvarelser er anonyme — navn kobles aldri til svar (GDPR).
          </p>
          <div className="flex gap-2">
            <input
              readOnly
              value={surveyUrl}
              className="flex-1 rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-2 font-mono text-sm text-neutral-600"
            />
            <button type="button" onClick={copyLink} className={BTN_PRIMARY}>
      ...
      <div className="rounded-lg border border-[#1a3d32]/20 bg-[#f4f1ea] px-4 py-3 text-sm text-[#1a3d32]">
        <strong>Personvern (GDPR):</strong> Besvarelser lagres uten navn eller e-post. En engangs-token brukes kun for å
        hindre dobbel besvarelse. Resultater vises aldri for grupper med færre enn {campaign.anonymity_threshold}{' '}
        respondenter.
      </div>
    </div>
  )
