...
export function AuthPage({ mode }: { mode: Mode }) {
  ...
  // OAuth (H3.4) — Google + Microsoft (Supabase 'azure' provider). The
  // provider redirects back to /login?redirect=…; the post-login session
  // handling is the same as password login. Profile name/email mapping is
  // handled by handle_new_user() (coalesces full_name → display_name).
  const signInWithProvider = async (provider: 'google' | 'azure') => {
    ...
    try {
      ...
      if (err) throw err
      // Browser navigates away on success — no further handling here.
    } catch (err) {
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={(e) => void submit(e)} className="mt-7 space-y-4">
            ...
            <div className="flex items-center gap-3" aria-hidden>
              ...
              <span className="text-[11px] uppercase tracking-wide text-neutral-400">eller</span>
              <span className="h-px flex-1 bg-neutral-200" />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className="flex items-center justify-center gap-2 rounded-lg border border-neutral-200 bg-white py-2.5 text-sm font-semibold text-neutral-800 transition hover:border-neutral-400 disabled:opacity-50"
              >
