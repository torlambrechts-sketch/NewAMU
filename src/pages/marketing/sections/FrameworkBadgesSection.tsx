      ...
      <div className="mx-auto max-w-6xl px-4 md:px-8">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          {FRAMEWORKS.map((f) => (
            <Link
              key={f.short}
              to={`/compliance#${f.slug}`}
              className="group flex items-baseline gap-2 rounded-full border px-4 py-2 text-sm transition-colors hover:bg-white/8"
              style={{ background: 'rgba(255,255,255,0.04)', borderColor: 'rgba(255,255,255,0.12)' }}
              aria-label={`${f.short} — ${f.paragraphs.length} paragrafer. Les mer på /compliance.`}
            >
