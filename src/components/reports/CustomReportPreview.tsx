    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      {modules.map((m) => {
        ...
        if (m.kind === 'table') {
          ...
          return (
            <div key={m.id} className={`${R} border border-neutral-200/90 bg-white p-5 shadow-sm lg:col-span-2`}>
              <p className="text-[10px] font-bold uppercase tracking-wider text-neutral-500">{m.title}</p>
          ...
          return (
            <div key={m.id} className={`${R} border border-neutral-200/90 bg-white p-5 shadow-sm lg:col-span-2`}>
              ...
              {segments.length ? (
                ...
                <p className="mt-4 text-xs text-neutral-500">Ingen numeriske felt i datasettet for diagram.</p>
              )}
            </div>
          )
        }
      ...
      {modules.length === 0 ? (
        <p className="text-sm text-neutral-500 lg:col-span-2">Legg til moduler i redigeringspanelet.</p>
      ) : null}
    </div>
  )
}
