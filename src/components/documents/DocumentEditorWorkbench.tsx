...
export function DocumentEditorWorkbench({
  ...
  // Resolve the page's source template. Prefers the proper FK column
  // `created_from_template_id` (added in migration 20260828120037);
  // falls back to the legacy convention key `metadata.__template_id`
  // for pages authored before the FK landed. The migration backfills
  // the FK from the convention key, so this fallback is only relevant
  // on databases where that migration hasn't been applied yet.
  const sourceTemplate = useMemo(() => {
    if (!originalPage) return null
    const fkId = originalPage.createdFromTemplateId ?? null
    const legacyId = (originalPage.metadata as { __template_id?: unknown } | undefined)?.__template_id
    const tplId = fkId ?? (typeof legacyId === 'string' ? legacyId : null)
            ...
            <div className="flex w-full flex-1 flex-col border-neutral-200/90 bg-white shadow-sm lg:border-r-0">
              <div className="flex-1 border-b border-neutral-100">
                <TipTapRichTextEditor
                  ...
                  className="rounded-none border-0 shadow-none [&_.tiptap-editor-root]:px-4 [&_.tiptap-editor-root]:py-6 sm:[&_.tiptap-editor-root]:px-8"
                />
              </div>
            </div>
          </div>
