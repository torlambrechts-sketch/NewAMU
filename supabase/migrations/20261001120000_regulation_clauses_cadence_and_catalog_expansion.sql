-- regulation_clauses: add recommended cadence + rationale, expand the
-- seed catalog so it matches frameworkParagraphs.ts (the krav enumerator
-- on the Internkontroll page).
--
-- Why this migration exists
-- ─────────────────────────
-- The Gap-analyse view renders a "Anbefalt løsning fra Klarert" card on every
-- krav row that lacks an active kontroll. Until now the recommended cadence +
-- legal rationale lived hard-coded in `src/pages/overview/internkontroll/
-- sections/internkontrollTokens.ts` (PARAGRAPH_CADENCE_OVERRIDES +
-- CATEGORY_CADENCE). The frontend code review caught this as a
-- maintainability problem — frequently-edited content does not belong in TS.
--
-- This migration moves both fields into the existing `regulation_clauses`
-- table so:
--   1. Org admins can adjust cadence per paragraph (still gated by RLS).
--   2. The data hook reads everything in one round-trip alongside title +
--      description (no extra query).
--   3. Future paragraphs only need to be added to the seed function — no
--      twin TS edit.
--
-- It also fills the 62 paragraphs that frameworkParagraphs.ts lists but the
-- previous seed didn't include (chapters 10/12/14A/15/16/18, IK-f § 1-4,
-- IK-f § 6-8, GDPR Art. 7/9/12, Åpenhetsloven § 1/3/6-8, ISO 45001
-- § 5.4 / § 7.2 / § 7.4 / § 9). Without these, gap rows for those
-- paragraphs render only the §-code with no title or description.
--
-- Arbeidstilsynet self-audit
-- ──────────────────────────
-- Pålegg-grunner addressed:
--   • Kap. 12 (Permisjoner) — uten seedede krav er det ingen rad å henge HR-
--     rutiner på i gap-analysen → arbeidsgiver kunne fått pålegg om manglende
--     skriftlig rutine etter AML § 3-1 jf. § 12-x.
--   • Kap. 18 (Tilsyn) — uten § 18-1/-7/-8/-10 i listen mangler eieren et
--     anker for "hvordan håndteres tilsyn/pålegg" → restrisiko ved tilsyn.
--   • Kap. 14A (Konkurranseklausuler) — ingen rad → ingen synlig kontroll
--     på riktig håndtering ved nyansettelser.
-- Restrisiko:
--   • Cadence-anbefalingene er Klarert-praksis, ikke lovbokstav for
--     paragrafer uten eksplisitt frekvens. UI skiller via cadence_rationale
--     IS NOT NULL ("lovgrunnlag") vs NULL ("Klarert-anbefaling").

-- ════════════════════════════════════════════════════════════════════════
-- 1. Schema additions
-- ════════════════════════════════════════════════════════════════════════

alter table public.regulation_clauses
  add column if not exists recommended_cadence text,
  add column if not exists cadence_rationale text;

comment on column public.regulation_clauses.recommended_cadence is
  'Klarert-anbefalt frekvens for kontrollen som etterlever kravet. Matcher '
  '`control_frequency_hint` enum-verdier (arlig | halvarlig | kvartalsvis | '
  'manedlig | ukentlig | daglig | ad_hoc). NULL = ingen anbefaling.';

comment on column public.regulation_clauses.cadence_rationale is
  'Legal basis or established practice that justifies the recommended_cadence. '
  'NULL betyr at frekvensen er en Klarert-heuristikk (ikke et lovkrav) — '
  'frontend UI skiller mellom de to slik at revisorer ikke siterer '
  'heuristikker som lovgrunnlag.';

-- Loose check — only accept the known frequency hints + NULL. We use a CHECK
-- (not a foreign-key reference to a separate table) because the same string
-- enum is mirrored in internal_controls.frequency_hint and we want to avoid
-- coupling the two tables.
do $$ begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'regulation_clauses_recommended_cadence_check'
  ) then
    alter table public.regulation_clauses
      add constraint regulation_clauses_recommended_cadence_check
      check (recommended_cadence is null or recommended_cadence in (
        'arlig', 'halvarlig', 'kvartalsvis', 'manedlig',
        'ukentlig', 'daglig', 'ad_hoc'
      ));
  end if;
end $$;

-- ════════════════════════════════════════════════════════════════════════
-- 2. Replace provision_regulation_clauses_baseline_for_org
-- ════════════════════════════════════════════════════════════════════════
-- Strategy: one INSERT … ON CONFLICT … DO UPDATE per framework. The on-
-- conflict clause sets every editable column so re-running the function
-- backfills cadence + rationale on existing rows in place (idempotent).
-- New paragraphs not present in the previous seed will be inserted fresh.
...
as $$
begin
  -- ─── AML — Arbeidsmiljøloven (172 → 83 paragraphs total now) ──────────
  ...
  on conflict (organization_id, id) do update set
    ...
    cadence_rationale = excluded.cadence_rationale,
  ...
  insert into public.regulation_clauses (
    ...
    ('ik-f-1',    p_org_id, 'ik-f', 'IK-f § 1',    'Formål', 'Fremme HMS-arbeid og forebygge svikt.',
     'arlig',     null, 290, true),
    ('ik-f-2',    p_org_id, 'ik-f', 'IK-f § 2',    'Virkeområde', 'Aktivitetene som omfattes av forskriften.',
     'arlig',     null, 291, true),
    ('ik-f-3',    p_org_id, 'ik-f', 'IK-f § 3',    'Internkontroll — definisjon', 'Hva internkontroll betyr.',
     'arlig',     null, 292, true),
    ('ik-f-4',    p_org_id, 'ik-f', 'IK-f § 4',    'Plikt til internkontroll', 'Hvem som har plikt til internkontroll.',
     'arlig',     'Ledelsens gjennomgang av plikten årlig.', 293, true),
    ('ik-f-5',    p_org_id, 'ik-f', 'IK-f § 5',    'Innhold i internkontrollen', 'Krav om dokumentert internkontrollsystem.',
     'arlig',     'IK-forskriften § 5 — systemet gjennomgås minst årlig.', 294, true),
    ('ik-f-5-1',  p_org_id, 'ik-f', 'IK-f § 5 nr. 1', 'HMS-mål', 'Skriftlige HMS-mål for virksomheten.',
     'arlig',     null, 295, true),
    ('ik-f-5-1a', p_org_id, 'ik-f', 'IK-f § 5 nr. 1a', 'HMS-mål skriftlig', 'Skriftlige HMS-mål.',
     'arlig',     null, 300, true),
    ('ik-f-5-1b', p_org_id, 'ik-f', 'IK-f § 5 nr. 1b', 'Organisasjon og ansvar', 'Skriftlig fordeling av ansvar.',
     'arlig',     null, 301, true),
    ('ik-f-5-1c', p_org_id, 'ik-f', 'IK-f § 5 nr. 1c', 'Kunnskap og opplæring', 'Sikre kompetanse blant ansatte.',
     'arlig',     null, 302, true),
    ('ik-f-5-1d', p_org_id, 'ik-f', 'IK-f § 5 nr. 1d', 'Arbeidstaker-medvirkning', 'Sikre medvirkning i HMS-arbeidet.',
     'arlig',     null, 303, true),
    ('ik-f-5-2',  p_org_id, 'ik-f', 'IK-f § 5 nr. 2',  'Kartlegging av farer', 'Kartlegging av farer og problemer.',
     'arlig',     'Risikokartlegging minst årlig.', 310, true),
    ('ik-f-5-3',  p_org_id, 'ik-f', 'IK-f § 5 nr. 3',  'Risikovurdering', 'Vurdere risiko og planlegge tiltak.',
     'arlig',     'Risikovurdering minst årlig.', 311, true),
    ('ik-f-5-4',  p_org_id, 'ik-f', 'IK-f § 5 nr. 4',  'Avviksrutine', 'Rutiner for avvik og korrigerende tiltak.',
     'arlig',     null, 312, true),
    ('ik-f-5-5',  p_org_id, 'ik-f', 'IK-f § 5 nr. 5',  'Systematisk overvåking', 'Overvåking av HMS-arbeidet.',
     'arlig',     null, 313, true),
    ('ik-f-5-6',  p_org_id, 'ik-f', 'IK-f § 5 nr. 6',  'Tiltak basert på risiko', 'Tiltaksplan basert på risiko.',
     'arlig',     null, 314, true),
    ('ik-f-5-7',  p_org_id, 'ik-f', 'IK-f § 5 nr. 7',  'Tilsyn med systemet', 'Rutine for tilsyn med IK-systemet.',
     'arlig',     'IK-forskriften § 5 nr. 7 — jevnlig overvåkning og gjennomgang.', 315, true),
    ('ik-f-5-8',  p_org_id, 'ik-f', 'IK-f § 5 nr. 8',  'Årlig gjennomgang', 'Årsgjennomgang av IK-systemet.',
     'arlig',     'Pliktig årsgjennomgang av IK-systemet.', 316, true),
    ('ik-f-6',    p_org_id, 'ik-f', 'IK-f § 6',    'Samordning', 'Samordning når flere virksomheter på samme sted.',
     'ad_hoc',    null, 320, true),
    ('ik-f-7',    p_org_id, 'ik-f', 'IK-f § 7',    'Tilsyn og sanksjoner', 'Tilsynsmyndighet etter forskriften.',
     'ad_hoc',    null, 321, true),
    ('ik-f-8',    p_org_id, 'ik-f', 'IK-f § 8',    'Klage', 'Klage på vedtak.',
     'ad_hoc',    null, 322, true)
  on conflict (organization_id, id) do update set
    code = excluded.code,
    title = excluded.title,
    description = excluded.description,
  ...
  insert into public.regulation_clauses (
    ...
    ('iso-9001-4',   p_org_id, 'iso-9001', 'ISO 9001:2015 § 4',   'Kontekst', 'Klausul 4: organisasjonens kontekst.',
     'arlig', null, 400, true),
    ('iso-9001-5',   p_org_id, 'iso-9001', 'ISO 9001:2015 § 5',   'Lederskap', 'Klausul 5: lederskap.',
     'arlig', null, 401, true),
    ('iso-9001-6',   p_org_id, 'iso-9001', 'ISO 9001:2015 § 6',   'Planlegging', 'Klausul 6: planlegging.',
     'arlig', null, 402, true),
    ('iso-9001-7',   p_org_id, 'iso-9001', 'ISO 9001:2015 § 7',   'Støtte', 'Klausul 7: støtte.',
     'arlig', null, 403, true),
    ('iso-9001-8',   p_org_id, 'iso-9001', 'ISO 9001:2015 § 8',   'Drift', 'Klausul 8: drift.',
     'manedlig', null, 404, true),
    ('iso-9001-9',   p_org_id, 'iso-9001', 'ISO 9001:2015 § 9',   'Evaluering av ytelse', 'Klausul 9: evaluering.',
     'arlig', null, 405, true),
    ('iso-9001-9-2', p_org_id, 'iso-9001', 'ISO 9001:2015 § 9.2', 'Internrevisjon', 'Internrevisjon.',
     'arlig', 'ISO 9001 § 9.2 — internrevisjon minst årlig.', 406, true),
    ('iso-9001-9-3', p_org_id, 'iso-9001', 'ISO 9001:2015 § 9.3', 'Ledelsens gjennomgang', 'Management review.',
     'arlig', 'ISO 9001 § 9.3 — ledelsens gjennomgang minst årlig.', 407, true),
    ('iso-9001-10',  p_org_id, 'iso-9001', 'ISO 9001:2015 § 10',  'Forbedring', 'Klausul 10: forbedring.',
     'ad_hoc', null, 408, true)
  on conflict (organization_id, id) do update set
...
-- Grants unchanged from the original migration (security definer).
