-- Seed Norwegian HMS inspection templates for all organizations.
-- Based on: AML (Arbeidsmiljøloven), Internkontrollforskriften § 5,
-- Stoffkartotekforskriften, Arbeidsutstyrsforskriften, Brann- og eksplosjonsvernloven.
-- Only inserts when the org has zero inspection_templates (safe to re-run).

do $$
declare
  v_org uuid;

  -- ── Template 1: Standard HMS-vernerunde ──────────────────────────────────
  v_std jsonb := jsonb_build_object('title', 'Standard HMS-vernerunde', 'items', jsonb_build_array(
    ...
    jsonb_build_object('key','f_002','label','Ventilasjon og inneklima er tilfredsstillende','fieldType','yes_no_na','hmsCategory','fysisk','lawRef','AML § 4-4','required',true,'helpText','Sjekk temperatur (18–22 °C), luft­kvalitet og at aggregat ikke er støyende.'),
    ...
    -- ERGONOMI (AML § 4-4, Bildeskjermforskriften)
    ...
    -- KJEMIKALIER (AML § 4-5, Stoffkartotekforskriften)
    ...
    -- BRANN (Brann- og eksplosjonsvernloven, IK-forskriften)
  ...
  -- ── Template 2: Brannsikkerhet og rømning ────────────────────────────────
  ...
  -- ── Template 3: Kjemikalie- og stoffhåndtering ───────────────────────────
  ...
  -- ── Template 4: Ergonomi og kontorarbeidsplass ───────────────────────────
  ...
  for v_org in select id from public.organizations loop
    if not exists (
      ...
      insert into public.inspection_templates
        (organization_id, name, checklist_definition, is_active)
      values
        (v_org, 'Standard HMS-vernerunde',          v_std,   true),
        (v_org, 'Brannsikkerhet og rømning',        v_brann, true),
        (v_org, 'Kjemikalie- og stoffhåndtering',   v_kjem,  true),
        (v_org, 'Ergonomi og kontorarbeidsplass',   v_ergo,  true);
    end if;
  end loop;
end;
$$;
