-- Permission for Arbeidsplassrapportering (workplace reporting hub).
-- Restores complete seed_default_roles_for_org (whistle migration had stripped several keys)
-- and adds module.view.workplace_reporting for admin + member.
...
begin
  ...
  if r_admin is not null then
    insert into public.role_permissions (role_id, permission_key) values
      ...
      (r_admin, 'module.view.workplace_reporting'),
      ...
      (r_admin, 'whistleblowing.committee')
  ...
  if r_member is not null then
    insert into public.role_permissions (role_id, permission_key) values
      ...
      (r_member, 'module.view.workplace_reporting'),
...
select rd.id, 'module.view.workplace_reporting'
from public.role_definitions rd
where rd.slug in ('admin', 'member')
on conflict (role_id, permission_key) do nothing;

-- Demo org (fixed UUID) — same pattern as demo_org_extend_permissions
insert into public.role_permissions (role_id, permission_key)
select rd.id, 'module.view.workplace_reporting'
from public.role_definitions rd
where rd.organization_id = '00000000-0000-4000-a000-000000000001'
  and rd.slug in ('admin', 'member')
on conflict do nothing;
