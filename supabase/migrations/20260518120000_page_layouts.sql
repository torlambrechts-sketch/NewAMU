...
-- Admins (platform_is_admin) can read/write all rows;
-- all authenticated users can read published rows.
...
drop policy if exists "page_layouts_read_published" on public.page_layouts;
...
-- Platform admins can read ALL layouts (including unpublished drafts).
drop policy if exists "page_layouts_admin_read_all" on public.page_layouts;
...
-- Platform admins can insert new layouts.
drop policy if exists "page_layouts_admin_insert" on public.page_layouts;
...
-- Platform admins can update any layout row.
drop policy if exists "page_layouts_admin_update" on public.page_layouts;
...
-- Platform admins can delete layout rows.
drop policy if exists "page_layouts_admin_delete" on public.page_layouts;
