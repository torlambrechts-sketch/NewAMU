-- Refactor: provision_compliance_baseline_for_org → dispatcher over helpers.
--
-- After commits 5.3 + 5.4 the master provision function is approaching
-- 800 LoC because every batch redefines the entire function (CREATE OR
-- REPLACE) just to add a few new templates. This migration extracts
-- per-batch helpers so the master function stays slim and each future
-- batch is a small data-only migration that:
--   1. CREATE OR REPLACE one helper function for its templates,
--   2. Adds a `perform` call to the master.
--
-- No behaviour change. Same idempotent inserts, same per-template
-- explicit tag mappings, same templates available. The backfill DO loop
-- is re-run at the end so existing orgs converge to the refactored
-- state (idempotent).

-- ── Helper: ISO 45001 baseline ─────────────────────────────────────────
...
-- ── Helper: AML baseline (vernerunde) ──────────────────────────────────
...
-- ── Helper: AML batch 1 — IK core (avvik + årsgjennomgang) ─────────────
...
-- ── Helper: AML batch 2 — Onboarding & opplæring ───────────────────────
...
-- ── Master function: dispatcher over helpers ───────────────────────────
