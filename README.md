...
- **/hse** — HMS / verneombud: **vernerunder** med AML-sjekkliste, **inspeksjoner** (malbaserte runder med dynamiske felt: ja/nei/N/A, tekst, tall, bilde; lokasjon/objekt; statusflyt; avvik med alvorlighetsgrad og frist; tidsplan «kjør forfalte»), **/hse/inspection-settings** (typer, maler, lokasjoner, roller, statuser, planer, avvik — JSON eksport/import), klassisk fritekst-inspeksjonslogg, **hendelser/nestenulykker**, AML-struktur, **revisjonslogg**. **Supabase:** sett `VITE_SUPABASE_URL` og `VITE_SUPABASE_ANON_KEY` (se `.env.example`), kjør migrasjon `supabase/migrations/20260202120000_app_hse_state.sql` — da synkes HSE til tabellen `app_hse_state`. Uten env: fallback `localStorage` `atics-hse-v2`.
