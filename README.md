...
1. Kjør SQL fra `supabase/migrations/20260401120000_org_structure.sql`, deretter **`20260402120000_rbac_invites.sql`**, **`20260402120100_org_creation_admin_roles.sql`**, og ved **500-feil på `/profiles`** i nettleseren: **`20260405120000_fix_profiles_rls_no_recursion.sql`** (fikser RLS-rekursjon som gir PostgREST 500). Ved **«Kunne ikke opprette organisasjon»** for org.nr. som allerede finnes: kjør **`20260405130000_create_org_join_if_exists.sql`** (kobler brukeren til eksisterende org i stedet for unikhetsfeil). **`20260405140000_org_rpc_user_facing_messages.sql`** oppdaterer unntakstekster i norsk fra RPC (valgfritt hvis klient allerede mapper engelsk). Ved **feil ved lagring av visningsnavn** i onboarding: kjør **`20260405150000_profiles_update_rls_split.sql`** (RLS for `UPDATE` på egen profilrad). Ved **`infinite recursion detected in policy for relation "profiles"`**: kjør **`20260405160000_profiles_rls_no_subquery_recursion.sql`** (bytter ut `EXISTS`-subspørringer mot `profiles` i RLS med `current_org_id()` / `is_org_admin()`).
