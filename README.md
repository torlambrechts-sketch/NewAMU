...
- **Navigation** — Two-level header: **Council** is a separate highlighted group on the left of the primary row (then a divider), followed by Members, Org health, HSE, **Internkontroll**, **Prosesser** (prosessbygger — egendefinerte steg med lenker til Tasks/moduler), Tasks, E-learning. **Section submenu** switches with the route. Home/project dashboard is via the **atics** logo.
