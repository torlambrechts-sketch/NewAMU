...
**Status:** Spec is [`specs/studio-builder.md`](specs/studio-builder.md) §13 signed off. Branch `claude/studio-builder-evaluation-bRNjX`: Phases 0+1+2a+2b+3 substrate complete. **18 migrations applied to remote DB, 2 edge functions deployed**. Test surface: vitest baseline + 16 tests (registry round-trip + embedder contract). CI pipeline at `.github/workflows/studio-ci.yml`. Outstanding before production rollout: real ISMS-consultant content for ISO 27001 v1.x.x (content-eng), customer-signal validation Task 0.9 (PL+sales, formally gates Phase 2+ commitments), manual pixel-diff for Task 0.3, 100-org RLS profile.
