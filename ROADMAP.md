...
| 8.32 | ⏸ | Live-room Realtime sync | Current implementation polls via manual refresh button (functional but not "live"). Supabase Realtime subscriptions for `meeting_live_sessions`, `meeting_votes`, `meeting_speaker_queue` deferred to H13. Customers running ≤20-attendee AMU meetings can use polling; competitors (Sherpany, BoardWise) offer realtime as commercial baseline. |
| 8.33 | ⏸ | External invitee public viewer page | Schema shipped (8.30); the public route `/meetings/external/<token>` + token-gated read access on agenda/attendees/protocol is Phase 3 UI follow-up. Inspector/BHT-guest workflow incomplete until built. |
| 8.34 | ⏸ | Stakeholder-digest dispatch | `meeting_digest_recipients` table shipped (8.30) but no `send-meeting-digest` edge function exists. Post-signing distribution to filtered audiences requires the function + a UI trigger from the Protokoll tab. |
| 8.35 | ⏸ | Live-session chair-disconnect recovery | If a chair closes the browser mid-meeting, `meeting_live_sessions.ended_at` stays null indefinitely. RPC `meetings_recover_live_session(meeting_id)` for chair-role force-end (e.g. after 2h idle) is deferred. Today this requires manual SQL by an admin. |
| 8.36 | ⏸ | COI declaration UI | `meeting_agenda_items.conflict_of_interest jsonb` exists; no UI surface. AMU/varslingsutvalg chair takes COI notes that are invisible to participants. Phase 4. |
| 8.37 | ⏸ | GDPR Art. 17 erasure for meeting tables | New tables (`meeting_votes`, `meeting_live_sessions`, `meeting_external_invitees`, `meeting_digest_recipients`) contain `member_id` / `cast_by_user_id`. Cross-module GDPR erasure helper to be added in line with the existing pattern from documents + compliance modules. |
| 8.38 | ⏸ | Vitest test framework | `useMeetings.ts` is now ~2000 lines with 60+ public methods; `meeting_vote_result()` + `meeting_parity_check()` are pure SQL functions with parameterised inputs — ideal vitest + pgTAP targets. No test framework installed in repo. Tracked as pre-existing tech debt (also 8.22). |
