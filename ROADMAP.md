...
Single `/studio` shell replacing the 7+ scattered template / wizard / dashboard editors. Two modes (Simple = preset+form, Advanced = inspector+canvas), one registry per scope. Phases 0–3 substrate + first real embedder shipped; Phase 4 marketplace remains gated on §11 milestones.

| # | Status | Item | Notes |
|---|---|---|---|
| 6.1 | ✅ | **Phase 0 — substrate** | 9/10 engineering tasks shipped: 0.1 studio_revisions + trigger · 0.2 permissions + sticky mode · 0.3 WidgetKindRegistry + renderer extraction · 0.4 studioRegistry + types + form gen + preset picker · 0.5 dashboard_layouts.kind CHECK · 0.6 compliance_notifications extension · 0.7 prebuild assertion · 0.8 useStudioRevision + useStudioMode hooks. 0.9 (customer-signal validation, **non-engineering** — PL+sales) still pending; **Phase 2+ engineering gates on it.** Task 0.3 pixel-diff verification still deferred. |
| 6.2 | ✅ | **Phase 1 — StudioPage shell + Simple mode** | StudioPage at `/studio` with ScopePicker (accent-banded cards) · ModeToggle · SimpleModeCards · AdvancedShell · PartnerOrgSwitcher. 7 scope files (compliance/survey/documents/learning/meetings/registers/dashboards) × 3 presets = 21 outcome-named cards. Telemetry sink + 7 event types. /compliance-studio redirects to /studio?scope=compliance (Task 0.10). Workflows scope correctly listed as deferred (`STUDIO_DEFERRED_MODULES`) until workflow-engine-review.md Phase A/B lands. **Not yet:** PalettePanel · CanvasFrame · PropertyInspector · PreviewPane · VersionTimeline · PublishBar · ConflictModal · AutosaveIndicator — those carry meaning only with the remaining 6 embedders. |
| 6.3 | ✅ | **Phase 2a — Advanced mode + pack authoring plumbing** | studio_packs + studio_pack_drafts + publish_studio_pack RPC (immutable-on-publish trigger) ✅. **PackEditor.tsx** at `/studio/pakker` — draft/save/publish/export/import ✅. **studio-pack-export + studio-pack-import** edge functions deployed (verify_jwt=true) ✅. **All 7 embedders shipped** wrapping each scope's existing editor inline in the studio shell (compliance · survey · documents · learning · meetings · registers · dashboards). |
| 6.4 | ✅ | **Phase 2b — ISO 27001 ships via studio (loop-closes proof)** | Two versions co-exist per org: v1.0.0-skeleton (17 control ids + law_refs) and **v1.1.0 with concrete Annex A bodies** (Norwegian-first 1-2 sentence descriptions, suggested artifact kind, cadence, GDPR/NIS2/AML cross-references, full §9.3 management-review agenda). Customers upgrade explicitly via PackEditor. Gap matrix returns 18 paragraphs per org (≥10 acceptance). **Real ISMS-consultant content** for production fine-tuning still pending. |
| 6.5 | ✅ | **Phase 3 — Customer + partner authoring** | 3.1 review_status across 4 tables ✅. 3.2 PartnerOrgSwitcher + app.active_partner_id GUC propagation via set_studio_partner_context RPC + useStudioOrgContext + resolveActiveOrgId() resolver ✅. 3.3 partner_admin RLS layering on 7 tables + perf indexes ✅. 3.4 30-day offboarding TTL + trigger + last_edited_by stamp + pg_cron registration ✅. **PartnerOffboardingPage.tsx admin UI** at `/admin/partners` ✅. **PlatformStudioGrantsPage** at `/platform-admin/studio-grants` for grant/revoke via the new grant_/revoke_studio_partner_admin RPCs ✅. 3.5 "Audited" badge ⏸ per spec open Q 9. |
| 6.6 | ⏸ | **Phase 4 — Marketplace** | Gated on §11 milestones (100 Pro/Enterprise customers + 20 partners). No preparatory work beyond the pack-as-portable-artifact shape — that's in place via studio_packs.manifest + the deployed export/import edge functions. |

**Status:** Spec is [`specs/studio-builder.md`](specs/studio-builder.md) §13 signed off. Branch `claude/studio-builder-evaluation-bRNjX`: Phases 0+1+2a+2b+3 + workflows-Phase-1.5 substrate complete; **clone-from-template loop end-to-end across all 8 scopes** (SystemTemplateBrowser + clone_studio_template RPC + CloneDeepLinkRedirect + compliance inline auto-open + studio.advanced grant). PublishBar/AutosaveIndicator/ConflictModal all wired with real consumers; ConflictModal exercises optimistic-lock check in PackEditor end-to-end. **25 migrations applied to remote DB, 2 edge functions deployed**. Test surface: vitest 31 tests (registry round-trip + embedder contract for all 8 scopes + resolveActiveOrgId + route smoke + import contract + clone-flow contract) + pgTAP 7 invariants (substrate + clone_studio_template). CI pipeline at `.github/workflows/studio-ci.yml`. RLS profile script + visual-diff fixture seeded. Customer-signal validation kit shipped at [`specs/studio-signal-validation.md`](specs/studio-signal-validation.md). Outstanding before production rollout: PL+sales runs the 8 interviews (Task 0.9 formally gates Phase 2+), ISMS-consultant fine-tunes ISO 27001 v1.x.x bodies for production use, eng captures the pixel-diff baseline against the seeded fixture, 100-org RLS profile against staging. Honest deferrals: useStudioAutosave consumer for the studio-aware tables (column shipped; needs embedder integration that lifts row-state) + builder canvas surfaces for non-compliance scopes (multi-sprint per scope).
