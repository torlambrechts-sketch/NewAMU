...
| 6.2 | ✅ | **Phase 1 — StudioPage shell + Simple mode** | Shipped: StudioPage at `/studio` with ScopePicker · ModeToggle · SimpleModeCards · AdvancedShell stub. 7 scope files (compliance/survey/documents/learning/meetings/registers/dashboards) × 3 presets = 21 outcome-named cards. Telemetry sink + 7 event types. /compliance-studio redirects to /studio?scope=compliance (Task 0.10). Workflows scope deferred until workflow-engine-review.md Phase A/B lands (spec §12). **Not shipped:** PalettePanel · CanvasFrame · PropertyInspector · PreviewPane · VersionTimeline · PublishBar · ConflictModal · AutosaveIndicator — those carry meaning only with real embedders (Phase 2a.1). |
| 6.3 | 🚧 | **Phase 2a — Advanced mode + pack authoring plumbing** | Substrate ✅: studio_packs + studio_pack_drafts + publish_studio_pack RPC (immutable-on-publish via trigger). studio-pack-export + studio-pack-import edge functions written (not yet deployed). **Not shipped:** real embedder adapters wrapping the 7 existing per-module editors (TipTap/dnd-kit/slide-panel/JSON-schema-form/DashboardEditLayoutPanel). PackEditor.tsx UI. The embedders are the explicit 2-sprint risk surface; current Phase 1 stubs link out to existing module admin pages. |
...
| 6.5 | 🚧 | **Phase 3 — Customer + partner authoring** | 3.1 review_status column on 4 remaining tables ✅. 3.2 PartnerOrgSwitcher ✅ (localStorage persistence — does NOT yet propagate to `app.active_partner_id` GUC on writes). 3.3 partner_admin RLS migration ✅ (conditional on partner_console_v0 substrate being applied). 3.4 partner offboarding TTL column + trigger ✅; PartnerOffboardingPage.tsx admin UI not shipped. 3.5 "Audited" badge ⏸ per spec open Q 9. |
| 6.6 | ⏸ | **Phase 4 — Marketplace** | Gated on §11 milestones (100 Pro/Enterprise customers + 20 partners). No preparatory work beyond the pack-as-portable-artifact shape — that's in place via studio_packs.manifest + the export/import edge functions. |

**Status:** Spec is [`specs/studio-builder.md`](specs/studio-builder.md) §13 signed off. Branch `claude/studio-builder-evaluation-bRNjX` has Phases 0+1 + Phase 2a/2b/3 substrate + 11 migrations applied to remote DB. Outstanding before production rollout: real embedders (Phase 2a.1), PackEditor UI (Phase 2a.2), edge function deploy (Phase 2a.3), ISO 27001 content drafting (Phase 2b), `app.active_partner_id` GUC propagation in partner switcher (Phase 3.2 polish), customer-signal validation (Task 0.9, gates Phase 2+ formally).
