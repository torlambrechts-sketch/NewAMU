...
| # | Status | Item | Notes |
|---|---|---|---|
| 5.1 | ✅ | Gap view per regulation | Live as `/overview/internkontroll/gaps` — paragraph × module heatmap + ParagraphInspectorPanel drill-down. Reads from `compliance_checklist_templates.law_refs[]`, `survey_template_catalog.law_refs[]`, `register_types.aml_paragraphs[]`, `document_system_templates.legal_basis[]`, `learning_*.law_refs`, plus the Tier-2 control coverage via `internal_control_clauses` join. |
| 5.2 | ✅ | Plan & timeline (Gantt-ish) | Live as `/overview/internkontroll/plan` — chapter-grouped lane view with status-coloured bars positioned by `start_at` → `due_at`. KPI strip + today marker + overdue chips. Auto-spawns `task_items` row when status flips to `in_progress` via the bridge in `useCompliancePlanItems`. |
| 5.3 | ✅ | Auditor view | Live as `/auditor/internkontroll/:token` + `/auditor/controls/:token`. Token mint/revoke UI at `AuditorTokensSection` (inline in gap-page + admin); print/PDF + frosset-snapshot band ship in the same surface. Mirror of `survey_invitation_tokens` pattern with bearer-secret hardening. |
| 5.4 | ✅ | Evidence ledger per § | Renders inside ParagraphInspectorPanel via `useParagraphEvidence`, reading `compliance_evidence_v` (Tier-3 union across compliance executions, meetings, document acks, learning completions, task completions, register records, surveys). Chronological list sorted by `occurred_at` desc. |
| 5.5 | ✅ | KPIs for ledelsen | Lead the HMS-oversikt composite — 4 KPI widgets (AML-dekning, åpne tilsynspålegg, ARP-status, AML-§-er uten plan) computed by `useLedelsesKpis` against `regulation_clauses` + `internal_control_clauses` + `register_records (aml_18_tilsynssaker)` + ARP wiki ack ledger + `compliance_plan_items`. |
...
| 8.26 | ✅ | Audit gap closure (Sprint-1) | Branch `claude/audit-meeting-module-efrov`. Migration `20260913121000_meetings_drop_forskrift_3_2_citation.sql` strips the regressed citation across all templates; `MeetingsDetailView.tsx` + `meetingsLegalReferences.ts` re-pointed to § 3-16. Migration `20260913121500_meetings_sign_protocol_rpc.sql` adds `meetings_sign_protocol_v1` (SECURITY INVOKER, atomic L1-event + signature + protocol-stamp); `signProtocol` consumes it via `supabase.rpc()` with client-side SHA-256 over the canonical agenda+attendees+decisions+signature payload. New edge function `send-meeting-invites/` (copy of `send-survey-invites` pattern — Resend + UTF-8-safe ICS attachment + `MEETINGS_CRON_SECRET` reminder sweep); `useMeetings.markInvitationSent` + `sendInvitations` hooks with partial-failure semantics; "Send innkalling" / "Send ny innkalling" / "Send påminnelse" buttons on Detail Informasjon-tab. Closes C1-C4. |
| 8.27 | ✅ | Audit gap closure (Sprint-2) | Migration `20260913121100_meetings_protocol_exports_auto_compute.sql` auto-computes `meeting_protocol_exports.payload_sha256` via pgcrypto trigger on `protocol_signed_at` transition + backfills already-signed meetings (hash now covers `minority_dissent_text`). Migration `20260913121050_meetings_minority_dissent.sql` adds the column for Forskrift om org. ledelse § 3-16 + UI textarea conditional on dissent votes (ordered before 121100 so the column exists when the protocol-export backfill runs). Migration `20260913121300_meetings_archived_at_enforcement.sql` adds BEFORE-UPDATE trigger + child-table write predicate so archived meetings become true soft-delete. Migration `20260913121400_meetings_workflow_rule_seeds.sql` seeds three rules consuming the existing `ON_MEETING_SCHEDULED/SIGNED/DECISION_LOGGED` events (innkalling-bekreft, neste-møte, vedtak-oppfølging). Dashboard scope gains `meeting_invitation_compliance` dataset + DONUT_INVITATION widget in default layout. Closes S1-S5. |
| 8.28 | ✅ | External-review closeout | Senior-dev review (P0: `btoa()` corrupted UTF-8 in ICS — replaced with `base64Utf8()`; optimistic stamp before email confirms — reordered to send-first-stamp-on-success). P1: RPC switched from SECURITY DEFINER to SECURITY INVOKER (DEFINER bypassed RLS — privilege escalation gap); server-side protocol-export hash now includes `minority_dissent_text` (was missing from both trigger and backfill). Design-agency review: "Send ny innkalling" copy fix, zero-participants disabled-with-tooltip, WarningBox for errors, "Påminnelse er nå tilgjengelig" hint, partial-failure surface in success toast, `role="status"` on transient feedback. |
| 8.29 | ✅ | Klarert-design Phase 1+2 | Voting models + RSVP + Live Meeting Room. Migrations `20260914120000_meetings_voting_models.sql` (5 voting models incl. AML § 7-1 (2) parity + `meeting_votes` + `meeting_vote_result()` RPC), `_120100_meetings_rsvp_substitute.sql` (RSVP state machine + side enum + substitute relation + `meeting_parity_check()` RPC), `_120200_meetings_live_sessions.sql` (live session + speaker queue). New page `src/pages/meetings/MeetingLivePage.tsx` (cyan #0891b2 full-screen workspace, agenda rail + voting + speaker queue + parity bar). 5 new components: `TimeBudgetBar`, `CadenceWarningCard` (wired into Hub), `ParityPanel` + `RsvpRosterEditor` (wired into Deltakere tab), `AutoSourceRail` (wired into Agenda tab). 18 new hook methods on `useMeetings`. |
| 8.30 | ✅ | Klarert-design Phase 3 schema | Migrations `_120300_meetings_external_invitees.sql` (token-gated invitees + digest recipients) + `_120400_meetings_three_role_signature.sql` (`required_signer_roles[]` + AFTER-INSERT trigger that flips `protocol_signed_at` only when all required roles signed — Forskrift § 3-16). |
| 8.31 | ✅ | Phase 2 hardening | Two external-review rounds. Round 1 (`_120500`): parity edge case (one side absent → `parity_missing_employer/employee` reason codes vs false-negative) + `organization_id` denorm on votes + speaker_queue. Round 2 (`_120600`): `meeting_votes.member_id NOT NULL` (NULL allowed unlimited duplicate ballots in PG unique indexes — corrupted tallies); `side` CHECK tightened to exclude `observer` from voting; token entropy upgraded from 96 to 128 bits in `addExternalInvitee`. Live-room timer now derives elapsed from persisted `started_at` (no zombie-session timer drift). |
| 8.32 | ✅ | Live-room Realtime sync | Migration `20260914121000_meetings_realtime_publication.sql` adds `meeting_votes`, `meeting_speaker_queue`, `meeting_live_sessions`, `meeting_attendees` to the `supabase_realtime` publication. `MeetingLivePage` subscribes via `supabase.channel().on('postgres_changes', ...)` and bumps `refreshKey` on inserts/updates — votes, taleliste, active-item changes, and RSVPs all push to the live workspace without manual refresh. Cleanup on unmount via `removeChannel`. |
| 8.33 | ✅ | External invitee public viewer page | New public route `/meetings/external/:token` rendered by `src/pages/meetings/MeetingExternalViewerPage.tsx`. Migration `20260914120900_meetings_external_token_redeem.sql` adds the `meetings_external_redeem_token(p_token text)` RPC (SECURITY DEFINER, anon-callable; validates token + expiry, stamps `used_at` on first redemption, returns meeting + agenda payload scoped to `access_level` — observers see structure only; speak/vote also see minutes + decisions). |
| 8.34 | ✅ | Stakeholder-digest dispatch | New edge function `send-meeting-digest` (mirrors send-meeting-invites Resend pattern). Renders the protocol or vedtak-only extract as HTML email and dispatches to recipients resolved from `recipient_filter` jsonb. `useMeetings.sendDigest()` hook + post-call sent_at/sent_count stamping per recipient row. AML § 7-2 (6) distribution loop closed. |
| 8.35 | ✅ | Live-session chair-disconnect recovery | Migration `20260914120700_meetings_live_session_recovery.sql` adds `meetings_recover_live_session(p_meeting_id uuid)` RPC (SECURITY INVOKER, requires `meetings.manage` or org-admin). Force-ends a zombie session by setting `ended_at = now()` where it's still null. Hook method `recoverLiveSession`. |
| 8.36 | ✅ | COI declaration UI | New `CoiDeclarePanel` component on `modules/meetings/components/CoiDeclarePanel.tsx`. Member-picker + reason textarea + per-entry remove; persists to `meeting_agenda_items.conflict_of_interest` jsonb via new hook method `setAgendaConflictOfInterest`. Component ships as a primitive — wiring into the agenda-item editor surface for routine use is a follow-up polish. |
| 8.37 | ✅ | GDPR Art. 15 + 17 helpers for meeting tables | Migration `20260914120800_meetings_gdpr_helpers.sql` adds two SECURITY INVOKER RPCs: `meetings_gdpr_pseudonymize_member(uuid)` redacts `signer_name`, nulls `notes`/`rsvp_reason`/`responsible_member_id`/speaker `topic`, preserves vote tallies (legal record per Forskrift § 3-16); `meetings_gdpr_export_member(uuid)` returns jsonb dump of every meeting-side row referencing the subject (Art. 15 access). Both require `meetings.manage_confidential` or org-admin. |
| 8.38 | ⏸ | Vitest test framework | Deliberately deferred to a dedicated infra PR — touches `package.json`, `tsconfig.test.json`, CI. Tracked as cross-module concern (same as 8.22). |
| 8.39 | ✅ | Research-report close-out v2 — schema | Migration `20261005120000_meetings_extension_v2_schema.sql`. (a) Voting-model CHECK widened with `aksje_simple_majority_one_third_floor` (aksjeloven § 6-25) + `weighted` (gf aksjeveid). (b) `meeting_votes.ballot_weight numeric` for aksjeantall. (c) `meetings.amu_leader_period_party` (arbeidsgiver|arbeidstaker) drives parity-tie double-vote per forskriftens § 3-15; locked post-sign. (d) `meetings.confidentiality_level` widened with `'akan'`; separate RLS predicate gated on new permission `meetings.view_akan` (separate perimeter from `manage_confidential`). (e) New table `meeting_reporting_obligations` + AFTER-INSERT trigger materialises statutory deadlines from `definition.reportingObligations[]`. (f) `meeting_vote_result()` rewritten — handles all 7 models, surfaces `third_floor` + `weighted_tally` + `leader_party` in payload, breaks AMU parity-tie using `meetings.amu_leader_period_party`. (g) `aksjelov` category seeded by provision fn. |
| 8.40 | ✅ | Research-report close-out v2 — 11 new templates | Migration `20261005120100_meetings_extension_v2_templates.sql`. Aksjeloven: `styremote-as` (§ 6-25 voting + § 6-29 dissens), `generalforsamling-ordinaer` (weighted + § 5-18 qualified 2/3), `bedriftsforsamling` (§ 6-35, min_employee_count=200). Hovedavtalen: `kontaktmote-tillitsvalgte-styret` (§ 9-13), `forhandlingsmote-lonn` (kap. 3 + arbtv § 16). AKAN: `akan-oppfolgingsmote` (default `akan`, helsepersonelloven § 21). AML: `vernerunde` (befaring), `drofting-15-1-individuell`, `drofting-15-2-masseoppsigelse` (8 obligatoriske + NAV/Arbeidstilsynet/30d), `drofting-10-3-arbeidsplan` (14d frist), `drofting-8-1-informasjon` (kvartalsvis, min_employee_count=50). Plus re-framing av `dialogmote-*` til `framework='Folketrygdloven'`. Total templates = 30. |
| 8.41 | ✅ | Research-report close-out v2 — frontend | `modules/meetings/{types, meetingsLabels, useMeetings, dashboards}`: new enums, voting-model + AMU-leder labels, vote-reason map, `castVote` accepts `weight`, new methods `setAmuLeaderPeriodParty`/`loadReportingObligations`/`mark/unmarkReportingObligationFulfilled`, new dataset `meeting_reporting_obligations_status`. Two new components: `MeetingAmuLeaderRotationBadge` + `MeetingReportingObligationsPanel`. `MeetingsDetailView` adds "Rapportering" tab (conditional) + AKAN badge variant + AMU leder-rotasjon picker on Deltakere tab. `MeetingLivePage` voting panel handles new models with aksjeantall input + § 6-25 1/3-gulv display + weighted-tally + parity-tie warning. `MeetingsHubView` gates AKAN templates behind new permission `meetings.view_akan`. `permissionKeys.ts` adds `meetings.view_akan` + `meetings.manage_reporting_obligations`. `Badge.tsx` gains `akan` (purple) variant. |
...
| 9.4 | 🚧 | Phase 3 — UI string extraction | ~4,000–4,500 strings across ~440 files, module by module. Done: `ShellHeaderWidgets` (`shell.header.*`, incl. first plural), the locale switcher, `RouteErrorBoundary` (`errors.*`, class-component pattern via `i18n.t`). Remaining: shell/nav bulk, every feature module, admin, validation messages, ~68 hardcoded `nb-NO` formatting sites → `lib/i18n/format.ts`. |
| 9.5 | 🚧 | Phase 4 — marketing site + SEO | Done: marketing chrome (`marketing.*` namespace — nav/footer/shell; `navigation.ts` link builders). Remaining: 7 page bodies + section components + `content/*.ts` prose, locale-prefixed `/en` routes, per-locale prerender + `hreflang` in `scripts/prerender-marketing.mjs`. |
| 9.6 | 🚧 | Phase 5 — server-side text | Done: `_shared/i18n.ts` (locale catalog + resolver + shared helpers); `send-meeting-invites` wired per-recipient as the reference. Remaining: `send-survey-invites`, `send-meeting-digest`, `documents-notification-digest` (same pattern); notification SQL → key+params. Government PDFs stay nb (legally locked). |
| 9.7 | 🚧 | RPC locale-parameterisation | Done: `set_profile_locale()` RPC + `handle_new_user()` whitelist widened to `app_locales` (migration `20260916121000`). Remaining: `p_locale` on the 4 provisioning RPCs (they default to nb today — enhancement, not a bug). |
