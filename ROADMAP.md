...
**Status:** Spec is [`specs/studio-builder.md`](specs/studio-builder.md) §13 signed off. Branch `claude/studio-builder-evaluation-bRNjX`: Phases 0+1+2a+2b+3 + workflows-Phase-1.5 substrate complete; spec-promised PublishBar + AutosaveIndicator + ConflictModal + studio_draft_payload + 24h-TTL purge shipped. **22 migrations applied to remote DB, 2 edge functions deployed**. Test surface: vitest 27 tests (registry round-trip + embedder contract for all 8 scopes + resolveActiveOrgId + route smoke + import contract) + pgTAP 6 invariants (revision trigger fires/skip GUC, immutable pack trigger, uniqueness, draft column coverage, permission key count). CI pipeline at `.github/workflows/studio-ci.yml`. RLS profile script + visual-diff fixture seeded. Customer-signal validation kit shipped at [`specs/studio-signal-validation.md`](specs/studio-signal-validation.md). Outstanding before production rollout: PL+sales runs the 8 interviews (Task 0.9 formally gates Phase 2+), ISMS-consultant fine-tunes ISO 27001 v1.x.x bodies for production use, eng captures the pixel-diff baseline against the seeded fixture, 100-org RLS profile against staging.
