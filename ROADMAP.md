...
| 11.15 | 📋 | Print stylesheet + PDF for auditor view | `@media print` rules on both auditor surfaces + browser-print "Lagre som PDF" interim. Server-side PDF + Merkle-chain checksum waits on generic `compliance-audit-pdf` from `specs/workflow-engine-review.md` Phase D. |
| 11.16 | 📋 | Watermark band on controls auditor | Match the internkontroll auditor's "FROSSET SNAPSHOT" treatment (the controls auditor has it inline in the header, not as a band). |
