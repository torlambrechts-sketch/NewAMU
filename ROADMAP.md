...
| 6.3 | 🚧 | **Phase 2a — Advanced mode + pack authoring plumbing** | Substrate ✅: studio_packs + studio_pack_drafts + publish_studio_pack RPC (immutable-on-publish via trigger). **PackEditor.tsx** shipped at `/studio/pakker` — draft / save / publish / export / import surface. **studio-pack-export** + **studio-pack-import** edge functions **DEPLOYED** to remote env (both verify_jwt=true). **First embedder shipped:** compliance scope wraps TemplateEditorPanel inline in `/studio?scope=compliance&mode=advanced` (~110 LoC adapter — canonical shape for the remaining 6). Remaining: survey · documents · learning · meetings · registers · dashboards embedders (Task 2a.1 follow-up). |
| 6.4 | 🚧 | **Phase 2b — ISO 27001 ships via studio** | Skeleton seeded: compliance_pack enum extended with iso-27001, studio_packs row per org carries 17 Annex A control law_refs (gap matrix ≥10 paragraph acceptance met). **Not shipped:** real control bodies (10-15 checklist templates, 5-8 documents, 3-5 e-learning courses, 2-3 surveys) — content-eng deliverable per spec open Q. |
| 6.5 | 🚧 | **Phase 3 — Customer + partner authoring** | 3.1 review_status column on 4 remaining tables ✅. 3.2 PartnerOrgSwitcher ✅ with **app.active_partner_id GUC propagation** via set_studio_partner_context RPC + useStudioOrgContext hook + resolveActiveOrgId() resolver wired into every preset mutator. 3.3 partner_admin RLS migration ✅ (conditional on partner_console_v0). 3.4 partner offboarding TTL column + trigger + last_edited_by stamp ✅; PartnerOffboardingPage.tsx admin UI not shipped. 3.5 "Audited" badge ⏸ per spec open Q 9. |
...
**Status:** Spec is [`specs/studio-builder.md`](specs/studio-builder.md) §13 signed off. Branch `claude/studio-builder-evaluation-bRNjX`: Phases 0+1+2a (substrate+plumbing+first embedder)+2b skeleton+3 substrate. **15 migrations applied to remote DB, 2 edge functions deployed.** Test baseline (vitest, 6 / 6 tests in src/lib/studio/) shipped. Outstanding before production rollout: 6 remaining embedders (Phase 2a.1 follow-up), ISO 27001 content drafting (Phase 2b, content-eng), PartnerOffboardingPage.tsx (Phase 3.4 admin UI), customer-signal validation (Task 0.9, formally gates Phase 2+).
