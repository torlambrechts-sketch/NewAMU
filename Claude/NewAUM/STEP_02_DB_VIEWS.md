# Step 2 — DB Migration: Views & Compliance Functions

Create file: `supabase/migrations/20260801010000_amu_redesign_views.sql`

Depends on: Step 1 (tables must exist).

---

## View 1: `amu_compliance_status`

One row per committee per year. All compliance flags for the Lovkrav card.
...
select
  ...
  -- parity: employer voting == employee voting
  (
  ...
  -- all voting members have valid HMS training
  not exists (
  ...
  -- annual report signed for previous year
  exists (
...
## View 2: `amu_meeting_summary`

Aggregates per meeting — used by Møteplan timeline.
...
## View 3: `amu_critical_queue`

Cross-module queue. Used by Oversikt KPI tile and Kritiske saker tab.

```sql
create or replace view public.amu_critical_queue as

-- unsigned completed meetings older than 14 days
select
...
-- draft annual reports for past years
select
  'draft_annual_report',
  ar.id,
  ar.organization_id,
...
-- meetings missing for current year vs min_meetings_per_year
select
  'missing_meeting',
  c.id,
  c.organization_id,
...
-- expired HMS training on voting members
select
...
> Note: `amu_meetings.completed_at` may not exist yet — add it in Step 1 if missing, or fall back to `updated_at` as shown above.
...
```
feat(amu): migration — compliance_status, meeting_summary, critical_queue views
```
